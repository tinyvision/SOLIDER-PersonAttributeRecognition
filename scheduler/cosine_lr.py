...
if __name__ == '__main__':
    ...
    optim = torch.optim.SGD(param_group, momentum=0.9, weight_decay=5e-4)
    ...
    for j in range(30):
        ...
        lr_s.append(optim.param_groups[1]['lr'])
        # lr_s.append(scheduler.get_last_lr()[0])
        optim.step()
        lr_scheduler.step(j+1)
    plt.plot(range(30), lr)
    plt.plot(range(30), lr_s)
    plt.show()
    print(lr)
